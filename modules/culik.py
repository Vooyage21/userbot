...
async def get_users(event):
    ...
    dantea = await eor(event, "`Processing....`")
    babi = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await dantea.edit("**Tidak bisa Menambahkan Member di sini.**")

    s = 0
    f = 0
    error = "None"
    await dantea.edit("`Processing...`")
